...
class _AddTrackAccountState extends State<AddTrackAccount> {
  ...
  Rx<List<SocialNetworksBlock>> socialNetworksTrackList = Rx<List<SocialNetworksBlock>>([SocialNetworksBlock(index: 0)]);
  TextEditingController firstNameController;
  TextEditingController lastNameController;

  getCountries() async {
    try {
      final _countries = await CountriesService.getCountries();

      for (Map<String, dynamic> country in _countries) {
  ...
  void initState() {
    super.initState();
    firstNameController = TextEditingController(text: widget.name != null ? widget.name.split(' ')[0] : '');
    lastNameController = TextEditingController(text: widget.name != null ? widget.name.split(' ')[1] : '');
    getCountries();
  }
                  ...
                  children: [
                    ...
                    Obx(
                      () => Column(
                        children: List.generate(
                          ...
                          (index) => Padding(
                            padding: EdgeInsets.only(bottom: index == socialNetworksTrackList.value.last.index ? 30 : 25),
                            child: Obx(
                    ...
                    Obx(
                      () => socialNetworksTrackList.value.length < 4
                          ? GestureDetector(
                              onTap: () {
                                socialNetworksTrackList.value.add(SocialNetworksBlock());
